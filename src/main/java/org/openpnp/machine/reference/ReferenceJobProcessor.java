...
public class ReferenceJobProcessor extends AbstractJobProcessor {
	 ...
	 * History:
	 * 
	 * Note: Can't actually use the @Version annotation because of a bug in SimpleXML. See http://sourceforge.net/p/simple/mailman/message/27887562/
	 * 
	...
	/*
	 * Pre-process the Job. We will: Look for setup errors. Look for missing parts. Look for missing feeders. Look for feeders that cannot feed the number of parts that will
	 * be needed. Calculate the base Safe-Z for the job. Calculate the number of parts that need to be placed. Calculate the total distance that will need to be traveled.
	 * Calculate the total time it should take to place the job.
	 * 
	 * Time calculation is tough unless we also ask the feeders to simulate their work. Otherwise we can just calculate the total distance * the feed rate to get close. This
	 * doesn't include acceleration and such.
	 * 
